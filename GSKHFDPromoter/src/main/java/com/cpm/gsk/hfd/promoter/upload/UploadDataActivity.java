                ...
                for (int i = 0; i < coverageBeanlist.size(); i++) {
                    if (!coverageBeanlist.get(i).getStatus().equalsIgnoreCase(CommonString.KEY_U)) {
                        String onXML = "[DATA][USER_DATA]"
                                ...
                                + "[/PROCESS_ID]"
                                // + "[EMP_ID]"
                                /*+ coverageBeanlist.get(i).getEmp_id()
                                + "[/EMP_ID]"
                                + "[PACK_ENABLE]"
                                + coverageBeanlist.get(i).getPakd_enable()
                                + "[/PACK_ENABLE]"*/
                                + "[CREATED_BY]"
                                + username
                                + "[/CREATED_BY]"
                       ...
                       /* // sales cycle data
                        final_xml = "";
                        onXML = "";
                        int value = Integer.parseInt(sales_cycle);
                        if (value > 0) {
                            onXML = "[SALES_CYCLE_DATA][MID]"
                                    + mid
                                    + "[/MID]"
                                    + "[CREATED_BY]"
                                    + username
                            ...
                            request = new SoapObject(CommonString.NAMESPACE, CommonString.METHOD_UPLOAD_XML);
                            request.addProperty("XMLDATA", sos_xml);
                            request.addProperty("KEYS", "SALES_CYCLE_DATA");
                            request.addProperty("USERNAME", username);
                            request.addProperty("MID", mid);
                            envelope = new SoapSerializationEnvelope(SoapEnvelope.VER11);
                            envelope.dotNet = true;
                            ...
                            if (!result.toString().equalsIgnoreCase(CommonString.KEY_SUCCESS)) {
                                ...
                                return "SALES_CYCLE_DATA";
                            }
                            data.value = 50;
                            data.name = "SALES_CYCLE_DATA";
                            publishProgress(data);
...
//////////////////////////////////////
                        ...
                        androidHttpTransport1.call(CommonString.SOAP_ACTION + CommonString.MEHTOD_UPLOAD_COVERAGE_STATUS,
                                envelope1);
                        ...
                        if (result1.toString().equalsIgnoreCase(CommonString.KEY_SUCCESS)) {
                            ...
                            database.updateStoreStatusOnLeave(coverageBeanlist.get(i).getStore_id(),
                                    coverageBeanlist.get(i).getVisit_date(), CommonString.KEY_U);
                        }
                        if (!result1.toString().equalsIgnoreCase(CommonString.KEY_SUCCESS)) {
